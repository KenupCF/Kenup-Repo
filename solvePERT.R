...
solvePERT<-function(Mode,Min,Max,Shape=4,
                    Conf=1,precision=100,link="identity",
  ...
  linkFUNs=list(identity=function(x){x},log=log)
  ilinkFUNs=list(identity=function(x){x},log=exp)
  ...
  if(Conf>1 | Conf<0){
    error<-TRUE
    # stop("Confidence level must be between 0-1")
  }
  if(is.na(Min) | is.na(Mode) | is.na(Max)){
    error<-TRUE
    # stop("Parameters Min, Mode and Max must be in increasing order")
  }else{
    if(Min> Max | Min> Mode | Max < Mode){
      error<-TRUE
      # stop("Parameters Min, Mode and Max must be in increasing order")
    }
    if(Min == Max & Mode == Max){
  ...
  # cat("\n defined error and point\n")
  if(!error & !point){
    
    Min=linkFUNs[[link]](Min)
    Max=linkFUNs[[link]](Max)
    Mode=linkFUNs[[link]](Mode)
    naturalMax=linkFUNs[[link]](naturalMax)
    naturalMin=linkFUNs[[link]](naturalMin)
  }
  
  if(is.na(Min) | is.na(Mode) | is.na(Max) | Min%in%c(Inf,-Inf) | Mode%in%c(Inf,-Inf) | Max%in%c(Inf,-Inf)){
    error<-TRUE
    # stop("Parameters Min, Mode and Max must be in increasing order")
  }
  ...
  if(error){
    resu<-list(trueMin=NA,trueMax=NA,RMSE=NA)
  }
  
  resu$trueMin=ilinkFUNs[[link]](resu$trueMin)
  resu$trueMax=ilinkFUNs[[link]](resu$trueMax)
